 ...
 * @api public
 */
export class MonitorAgent extends EventEmitter implements IMonitorAgent {
	reqId: number
	opts: IMonitorAgentOpts
	id: ServerId
	type: ServerType
	socket: MqttClient
	callbacks: {[reqId: number]: Callback<any>}
	info: IServerInfo
	state: number
	consoleService: ConsoleService

	constructor(opts: IMonitorAgentOpts) {
		super()
		this.reqId = 1;
		this.opts = opts;
		this.id = opts.id;
	 ...
	 * @api public
	 */
	connect(port: number, host: string, cb: Callback<void>) {
	 ...
	 * @api public
	 */
	notify<T>(moduleId: string, msg: T) {
	...
	request<T1, T2>(moduleId: string, msg: T1, cb: Callback<T2>) {
		if (this.state !== ST_REGISTERED) {
