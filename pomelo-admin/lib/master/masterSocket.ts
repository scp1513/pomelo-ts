...
import {IAuthSrvMsg} from '../consoleService'
...
class MasterSocket {
	...
	constructor(agent: MasterAgent, socket) {
		this.agent = agent;
		this.socket = socket;
	}

	onRegister(msg: IAuthSrvMsg) {
		if (!msg || !msg.type) {
			return;
		}
	...
	onReconnect(msg, pid) {
		...
		var record = this.agent.addConnection(serverId, msgServerType, msg.pid, msg.info, socket);
