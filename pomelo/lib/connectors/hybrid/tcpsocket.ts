import net = require('net')
import Stream = require('stream')
import {Package} from '../../../../pomelo-protocol'
var logger = require('pomelo-logger').getLogger('pomelo', __filename)

/**
 * Work states
 */
var ST_HEAD = 1;      // wait for head
var ST_BODY = 2;      // wait for body
var ST_CLOSED = 3;    // closed

let HEAD_SIZE = 4;

export interface ITcpSocketOpts {
	closeMethod?: string
}

/**
 * Tcp socket wrapper with package compositing.
 * Collect the package from socket and emit a completed package with 'data' event.
 * Uniform with ws.WebSocket interfaces.
 *
 * @param {Object} socket origin socket from node.js net module
 * @param {Object} opts   options parameter.
 *                        opts.headSize size of package head
 *                        opts.headHandler(headBuffer) handler for package head. caculate and return body size from head data.
 */
export class Socket extends Stream {
	readable: boolean
	writeable: boolean
	_socket: net.Socket
	closeMethod: string
	headBuffer: Buffer
	headOffset: number
	packageOffset: number
	packageSize: number
	packageBuffer: Buffer
	state: number

	constructor(socket: net.Socket, opts: ITcpSocketOpts) {
		super()
		if (!socket || !opts) {
			throw new Error('invalid socket or opts');
		}

		// stream style interfaces.
		// TODO: need to port to stream2 after node 0.9
		this.readable = true;
		this.writeable = true;

		this._socket = socket;
		this.closeMethod = opts.closeMethod;
		this.headBuffer = new Buffer(HEAD_SIZE);

		this.headOffset = 0;
		this.packageOffset = 0;
		this.packageSize = 0;
		this.packageBuffer = null;

		// bind event form the origin socket
		this._socket.on('data', (chunk) => this.ondata(chunk));
		this._socket.on('end', (chunk?: Buffer) => this.onend(chunk));
		this._socket.on('error', this.emit.bind(this, 'error'));
		this._socket.on('close', this.emit.bind(this, 'close'));

		this.state = ST_HEAD;
	}

	send(msg: string|Buffer, encode: string, cb: Callback<void>) {
		this._socket.write(msg, encode, cb);
	}

	close() {
		if (!!this.closeMethod && this.closeMethod === 'end') {
			this._socket.end();
		} else {
			try {
				this._socket.destroy();
			} catch (e) {
				logger.error('socket close with destroy error: %j', e.stack);
			}
		}
	}

	private ondata(chunk: Buffer) {
		if (this.state === ST_CLOSED) {
			throw new Error('socket has closed');
		}

		if (!Buffer.isBuffer(chunk)) {
			throw new Error('invalid data');
		}

		var offset = 0, end = chunk.length;

		while (offset < end && this.state !== ST_CLOSED) {
			if (this.state === ST_HEAD) {
				offset = this.readHead(chunk, offset);
				if (this.packageSize === HEAD_SIZE) {
					// 有些包可能会没有body
					offset = this.readBody(chunk, offset);
				}
			}

			if (this.state === ST_BODY) {
				offset = this.readBody(chunk, offset);
			}
		}

		return true;
	}

	private onend(chunk?: Buffer) {
		if (chunk) {
			this._socket.write(chunk);
		}

		this.state = ST_CLOSED;
		this.reset();
		this.emit('end');
	}

	/**
	 * Read head segment from data to socket.headBuffer.
	 *
	 * @param  {Object} data   Buffer instance
	 * @param  {Number} offset offset read star from data
	 * @return {Number}        new offset of data after read
	 */
	private readHead(data: Buffer, offset: number) {
		var hlen = HEAD_SIZE - this.headOffset;
		var dlen = data.length - offset;
		var len = Math.min(hlen, dlen);
		var dend = offset + len;

		data.copy(this.headBuffer, this.headOffset, offset, dend);
		this.headOffset += len;

		if (this.headOffset === HEAD_SIZE) {
			// if head segment finished
			var size = this.headHandler(this.headBuffer);
			if (size < 0) {
				throw new Error('invalid body size: ' + size);
			}
			// check if header contains a valid type
			if (this.checkTypeData(this.headBuffer[0])) {
				this.packageSize = size + HEAD_SIZE;
				this.packageBuffer = new Buffer(this.packageSize);
				this.headBuffer.copy(this.packageBuffer, 0, 0, HEAD_SIZE);
				this.packageOffset = HEAD_SIZE;
				this.state = ST_BODY;
			} else {
				dend = data.length;
				logger.error('close the connection with invalid head message, the remote ip is %s && port is %s && message is %j', this._socket.remoteAddress, this._socket.remotePort, data);
				this.close();
			}

		}

		return dend;
	}

	/**
	 * Read body segment from data buffer to socket.packageBuffer;
	 *
	 * @param  {Object} data   Buffer instance
	 * @param  {Number} offset offset read star from data
	 * @return {Number}        new offset of data after read
	 */
	private readBody(data: Buffer, offset: number) {
		var blen = this.packageSize - this.packageOffset;
		var dlen = data.length - offset;
		var len = Math.min(blen, dlen);
		var dend = offset + len;

		data.copy(this.packageBuffer, this.packageOffset, offset, dend);

		this.packageOffset += len;

		if (this.packageOffset === this.packageSize) {
			// if all the package finished
			var buffer = this.packageBuffer;
			this.emit('message', buffer);
			this.reset();
		}

		return dend;
	}

	private reset() {
		this.headOffset = 0;
		this.packageOffset = 0;
		this.packageSize = 0;
		this.packageBuffer = null;
		this.state = ST_HEAD;
	}

	private checkTypeData(data: number) {
		return data === Package.TYPE_HANDSHAKE ||
			data === Package.TYPE_HANDSHAKE_ACK ||
			data === Package.TYPE_HEARTBEAT ||
			data === Package.TYPE_DATA ||
			data === Package.TYPE_KICK;
	}

	private headHandler(headBuffer: Buffer): number {
		var len = 0;
		for (var i = 1; i < 4; i++) {
			if (i > 1) {
				len <<= 8;
			}
			len += headBuffer.readUInt8(i);
		}
		return len;
	}
	
}
